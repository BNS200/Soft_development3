...
int main(int argc, char *argv[]){
    ...
    auto customCPU = make_shared<IntelProcessor>(
        make_shared<double>(5.0),
        make_shared<ProcessorType>(x86_64),
        make_shared<std::string>("Custom CPU")
    ...
    gContainer.RegisterInstance<std::string>(std::make_shared<std::string>("Core i7"));

    gContainer.RegisterFactory<IProcessor, IntelProcessor, double, ProcessorType, std::string>();
    ...
    gContainer.RegisterInstance<std::string>(std::make_shared<std::string>("Core i9"));
